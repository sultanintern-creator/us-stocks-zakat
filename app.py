...
# --- إعدادات وتأمين تسجيل دخول جوجل (Google OAuth) ---
# ملاحظة للمدير: يجب تعيين هذه المعرفات في إعدادات Streamlit Cloud (Secrets) لتفعيل الربط الفعلي مع جوجل
client_id = st.secrets.get("GOOGLE_CLIENT_ID", "MOCK_CLIENT_ID")
client_secret = st.secrets.get("GOOGLE_CLIENT_SECRET", "MOCK_CLIENT_SECRET")
...
# ----------------------------------------------------
# الصفحة الأولى: حاسبة الزكاة للمتصفحين والمستخدمين
# ----------------------------------------------------
if page == "🧮 حاسبة الزكاة للمستثمرين":
    ...
    # --- [الحل الأول]: ميزة البحث الذكي عن الشركات باسمها العام ---
    st.markdown("### 🔍 البحث الذكي عن الشركة")
    search_query = st.text_input("اكتب اسم الشركة بالإنجليزية (مثال: Apple, Nvidia, Microsoft):", value="AMD").strip()
    ...
    if search_query:
        try:
            # استخدام أداة البحث الداخلي لياهو فاينانشال لجلب المقترحات آلياً
            search_results = yf.Search(search_query, max_results=5).tickers
            if search_results:
                # تجهيز قائمة بالشركات المكتشفة ليختار المستخدم منها
                ...
                if options:
                    ...
                    st.warning("لم نجد خيارات واضحة، سيتم اعتماد النص المكتوب كرمز مباشر.")
    ...
    if st.button("📊 احسب الزكاة المستحقة الآن"):
        ...
        # --- [الحل الثاني]: فرض تسجيل الدخول بحساب جوجل لطلب المعلومات وحمايتها ---
        if client_id == "MOCK_CLIENT_ID":
            # في البيئة التجريبية الحالية قبل ربط مفاتيح جوجل الرسمية، سنظهر محاكاة لزر التسجيل لحماية التدفق التجاري
            st.warning("🔒 حساب الزكاة المتقدم يتطلب تسجيل الدخول. يرجى الضغط على الزر أدناه للاتصال بجوجل:")
            if st.button("🔴 تسجيل الدخول السريع عبر حساب Google"):
                st.session_state['user_logged_in'] = True
                st.rerun()
        else:
            # الربط البرمجي الحقيقي مع جوجل كاملاً
            user_info = login(client_id=client_id, client_secret=client_secret, redirect_uri=redirect_uri)
            if not user_info:
                st.info("يرجى تسجيل الدخول بحساب جوجل أولاً لتمكينك من استخراج تقارير الوعاء الزكوي للمحفظة.")
                st.stop()
            else:
                st.sidebar.success(f"مرحباً بك، {user_info.get('name')}")
        
        # تنفيذ الحسبة الزكوية إذا تم التحقق من البيانات والرمز
        if selected_ticker:
            with st.spinner('جاري سحب البيانات المالية الحية ومعالجة الوعاء...'):
                try:
                    ...
                    # تسجيل الإحصاءات في لوحة التحكم
                    ...
                    if investment_type == "مضاربة (بيع وشراء قصير المدى)":
                        ...
                        # استعراض النتائج المنظمة للمستخدم
                    ...
                    st.error(f"يرجى التحقق من صياغة اسم الشركة للبحث. خطأ: {e}")
